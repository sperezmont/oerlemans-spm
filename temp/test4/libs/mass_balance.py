...
def calc_rgr(bed_type, hx_model, r, s, d0, mu):
    if bed_type == 'linear':
        if hx_model == 'plastic':
            return r - 1/mu * (s*r - d0)**2
...
def calc_MB(marine, bed_type, hx_model, r, zr, d0, s, mu, A, beta, f, delta, eta):
    if marine:
        if bed_type == 'linear':
            if hx_model == 'plastic':
                rgr = calc_rgr(bed_type, hx_model, r, s, d0, mu)
...
def calc_M(marine, bed_type, hx_model, r, s, d0, mu, eta, eps1, eps2):
    if marine:
        if bed_type == 'linear':
            if hx_model == 'plastic':
                M = pi*(1+eps1)*(4/3*np.sqrt(mu)*r**(3/2)-s*r**2) - \
                    2*eps2*pi*(s*r**2 - (d0-eta)*r)
